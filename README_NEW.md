...
```
ML-NLP/
├── finance_news_crawler/        # 多源财经快讯采集引擎
│   ├── clients/                  # 金十数据、汇通财经、财联社等5源
│   ├── common/                   # 两层去重、工具函数
│   ├── config.py
│   └── main.py
│
├── event_ontology/               # 事件本体与抽取（核心）
│   ├── event_ontology.json       # 10类事件本体定义（触发词+程度词+方向）
...
# 第三步：人工抽检（见 event_ontology/human_reviewer.py）
```
